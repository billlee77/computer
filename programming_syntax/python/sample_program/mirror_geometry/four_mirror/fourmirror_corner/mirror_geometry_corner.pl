...
sub corner_cal{
	...
	my ($mirror_number, $x_width, $y_width, $x_rad, $y_rad, $radius, @corner_pos) = @_;

	my $x = $corner_pos[0];
	my $y = $corner_pos[1];
	my $z = $corner_pos[2];

	#print "Value Test: ", $x_rad, " ", $y_rad,  " ", $radius, " ", $x, " ", $y, " ", $z, "\n";
	...
	$x_coord[0] = sprintf("%.2f", $x); 
	$y_coord[0] = sprintf("%.2f", $y);
	$z_coord[0] = sprintf("%.2f", $z);
	...
	if ($mirror_number == 3 || $mirror_number ==4){		
		$x_coord[1] = $x - $x_width * cos($x_rad);
 		$z_coord[1] = $z - $x_width * sin($x_rad) ;
	} else {
		$x_coord[1] = $x + $x_width * cos($x_rad);
		$z_coord[1] = $z + $x_width * sin($x_rad) ;
	}
...
# 	if ($mirror_number == 2 || $mirror_number ==4){		
# 		$z_coord[1] = $z - $x_width * sin($x_rad) ;
# 	} else {
# 		$z_coord[1] = $z + $x_width * sin($x_rad) ;
# 	}

	$x_coord[1] = sprintf("%.2f", $x_coord[1]); 
	$y_coord[1] = sprintf("%.2f", $y_coord[1]);
	$z_coord[1] = sprintf("%.2f", $z_coord[1]);
	#print "The Corner 2 of the mirror is at (x, y, z) (mm): ", $x_coord[2], "    ", $y_coord[2], "    ", $z_coord[2], "\n";



	# Corner 3
	$x_coord[2] = $x -  $y_width * sin($x_rad) * sin($y_rad);

	if ($mirror_number ==2 || $mirror_number == 4){
		$y_coord[2] = $y - $y_width * cos($y_rad);
	} else {
		$y_coord[2] = $y + $y_width * cos($y_rad);
	}
	#$z_coord[3] = $z + $x_width * cos($x_rad) * sin($y_rad) ;
	$z_coord[2] = $z + $y_width * cos($x_rad) * sin($y_rad) ;
	...
	if ($mirror_number ==3 || $mirror_number == 4){
		$x_coord[3] = $x - $x_width * cos($x_rad) - $y_width * sin($x_rad) * sin($y_rad);
		$z_coord[3] = $z + $y_width * cos($x_rad) * sin($y_rad) - $x_width * sin($x_rad) ;
	} else {
		$x_coord[3] = $x + $x_width * cos($x_rad) - $y_width * sin($x_rad) * sin($y_rad);
		$z_coord[3] = $z + $y_width * cos($x_rad) * sin($y_rad) + $x_width * sin($x_rad) ;
	}
...
sub para_cal{
	...
	my ($x_width, $y_width, $radius, $thickness) = @_;
	...
	return($z_depth, $small_dist_x, $dist, $length);
...
sub MirrorPos{
	...
	my ($mirror_number, $x_width, $y_width,$z_depth, $x_rad, $y_rad, $small_dist_x, $dist, $corner_temp_in, $x_coord_in, $y_coord_in, $z_coord_in) = @_;
	# my($x_width, $y_width, $x_rad, $y_rad, @corner_pos, $small_dist_x, $dist, @x_coord, @y_coord, @z_coord ) = @_;
#	my(@test) = @_;

	my @corner_pos = @$corner_temp_in;
	my @x_coord = @$x_coord_in;
	my @y_coord = @$y_coord_in;
	my @z_coord = @$z_coord_in;
...
#	"    Distx: ", $distx, "    Disty: ", $disty , "    Distz: ", $distz , "\n";
	...
	if ($mirror_number ==2 || $mirror_number == 4){
		 $y_center_start = - abs($y_coord[3] - $y_coord[1]) / 2.0 + $y;
	}
	...
	if(($mirror_number ==3 || $mirror_number == 4)){
	 	$MirrorPosition_x = - abs($x_center_end - $x_center_start)/2.0 - $small_dist_x * sin($x_rad) * cos($y_rad) + $x_center_start;
	}
...
#	print "Mirror Center Position at the x depth from Surface (x, y, z) (mm): ", $MirrorFix_x, "  ", $MirrorFix_y, "  ", $MirrorFix_z , "\n";
	...
	$x_center_start  = sprintf("%.2f", $x_center_start ); 
	$y_center_start  = sprintf("%.2f", $y_center_start );
	$z_center_start  = sprintf("%.2f", $z_center_start );

	$x_center_end = sprintf("%.2f", $x_center_end); 
	$y_center_end = sprintf("%.2f", $y_center_end);
	$z_center_end = sprintf("%.2f", $z_center_end);

	$MirrorPosition_x = sprintf("%.2f", $MirrorPosition_x ); 
	$MirrorPosition_y = sprintf("%.2f", $MirrorPosition_y );
	$MirrorPosition_z = sprintf("%.2f", $MirrorPosition_z );


	return( $x_center_start, $y_center_start, $z_center_start, 
            $x_center_end, $y_center_end, $z_center_end,
		    $MirrorPosition_x, $MirrorPosition_y, $MirrorPosition_z,);
