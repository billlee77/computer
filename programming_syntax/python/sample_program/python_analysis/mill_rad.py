...
# 	itt =0
# 	for m,n  in zip(coor_x, coor_y):
# 		#print itt, i
# 		m = m*2.54
# 		if m < 56 and m > 4:	
# 			data_select_x.append(m) 
# 			#print m, n  
# 	
# 	print data_select_x	
...
# 75% inner fit
#			if coor_x[i] < 56 or coor_x[i] > 4 or coor_y[i] < 51 or coor_y[i] > 4:
...
# 75% outer fit
#			if num_x < 40 and num_x > 560) and ( num_y < 40 and num_y > 510):
...
#x_out = numpy.array(data_select_x)
#y_out = numpy.array(data_select_y)
...
# No cut
		if x:
# 75% cut!
#		if x < 56 and x > 4 and y < 51 and y > 4:
# 50% cut!
#		if x < 52.5 and x > 7.5 and y < 46.5 and y > 8.5:
...
f2 = TF2("f2", "-(((x-[0])**2 + (y-[1])**2)/([2] + sqrt([2]**2 - (1+[3])* ((x-[0])**2 + (y-[1])**2))) -[4])", x_min, x_max, y_min, y_max)
